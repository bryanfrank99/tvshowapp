...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [p, l, c] = await Promise.all([
      sb.from("providers").select("id,name,needs_tmdb,tv_ok").eq("active", true).order("ord"),
    ...
    return NextResponse.json({
      providers: (p.data || []).map((x: any) => ({ id: x.id, name: x.name, needsTmdb: !!x.needs_tmdb, tvOk: !!x.tv_ok })),
