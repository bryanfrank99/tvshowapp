  ...
  return (
    <div className="grid md:grid-cols-[220px_1fr] gap-6">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-400 mt-1">⭐ {Math.round((m.vote_average || 0) * 10) / 10} · {m.release_date} · {m.runtime} min
        ...
        <div className="mt-4 flex gap-2 flex-wrap items-center"><Link href={`/watch?type=movie&id=${m.id}`} className="bg-violet-600 hover:bg-violet-500 rounded-xl px-5 py-2.5 font-bold inline-block">▶ Ver ahora</Link><FavButton big type="movie" id={params.id} title={m.title} poster={img(m.poster_path)} /></div>
      </div>
    </div>
  );
}
