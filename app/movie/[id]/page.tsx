  ...
  return (
    <div className="grid md:grid-cols-[220px_1fr] gap-6">
      ...
      <div>
        ...
        <p className="mt-3 text-zinc-300">{m.overview || d.sin_sinopsis}</p>
        ...
        <div className="mt-4 flex gap-2 flex-wrap items-center"><Link href={`/watch?type=movie&id=${m.id}`} className="bg-[#008CFF] hover:bg-[#008CFF] rounded-xl px-5 py-2.5 font-bold inline-flex items-center gap-2"><IconPlay size={15} />{d.ver_ahora_btn}</Link><FavButton big type="movie" id={params.id} title={m.title} poster={img(m.poster_path)} /></div>
      </div>
    </div>
  );
}
