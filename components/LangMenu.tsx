...
export default function LangMenu() {
  ...
  return (
    <div ref={ref} className="relative shrink-0" title={d.lang_label}>
      <button onClick={() => setOpen((v) => !v)} aria-label={d.lang_label} aria-haspopup="menu" aria-expanded={open} style={{ height: 36 }}
        className="px-3 sm:px-4 inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/10 text-sm font-bold text-zinc-300 hover:text-white hover:border-white/30 box-border">
        <IconGlobe size={16} />
