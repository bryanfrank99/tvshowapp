...
assert.equal(resolvedEs.sources.length, 4, "Debe resolver las 4 fuentes disponibles");
assert.equal(resolvedEs.sources[0].providerId, "vidcore", "Para usuario 'es', VidCore debe ser S1");
...
// 3. Verificamos que para usuario portugués RedeFlix se priorice primero
console.log("\n[3/4] Verificando afinidad idiomática dinámica en el Resolver...");
...
assert.equal(resolvedPt.sources[0].providerId, "redeflix", "Para usuario 'pt', RedeFlix debe tener máxima prioridad");
console.log("  ✅ RedeFlix priorizado dinámicamente para usuario 'pt'");
