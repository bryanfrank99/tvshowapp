...
async function runTests() {
  ...
  const stepTime = (msg) => {
    ...
    console.log(`⏱️  [+${elapsed}ms] ${msg}`);
  };

  console.log("\n1. Creando claves de prueba en Supabase...");
  stepTime("Inicio");
  const [{ data: codeA, error: errA }, { data: codeB, error: errB }] = await Promise.all([
    sb
      .from("access_codes")
      .insert({
        code_hash: codeAHash,
  ...
  stepTime("Claves creadas en paralelo");
  ...
  try {
    ...
    stepTime("Vinculación Clave A completada");
    ...
    stepTime("Update Clave A vencida");
    ...
    stepTime("Update Clave A revocada");
    ...
    stepTime("Reset sessions completado");

    // Vencer Clave A nuevamente
    await sb.from("access_codes").update({ expires_at: pastExp }).eq("id", codeA.id);
    stepTime("Update Clave A vencida post-reset");
    ...
    try {
      await Promise.all([
        sb.from("access_codes").delete().in("id", [codeA?.id, codeB?.id].filter(Boolean)),
        sb.from("config").delete().eq("key", `dev_bind:${testDevId}`)
      ]);
      stepTime("Limpieza finalizada");
    } catch {}
  }
}
